...
const friendshipSchema = new mongoose.Schema({
    relationship_id: { type: Number, unique: true },
    user_id: { type: mongoose.Schema.Types.ObjectId, ref: "User", required: true }, // Use ObjectId
    friend_id_or_follow_id: { type: mongoose.Schema.Types.ObjectId, ref: "User", required: true }, // Use ObjectId
    status: { type: String, enum: ["Pending", "Accepted", "Blocked"], required: true },
    request_sent: { type: Date, default: Date.now }
});
